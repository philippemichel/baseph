...
la fonction `listrandph`correspond juste à une mise en forme de la focntion `blockrand` du package homonyme pour générer des listes de randomisation par centre en blocs de taille aléatoire. ELle génère plusieurs fichiers .csv : une feuille globale de randomisation (pour le promoteur) & une feuille par centre.
...
J'utilise habituellement le package `gtsummary` pour créer mes tableaux.  Les deux fonctions suivantes prnnet un tableau, le mettent en forme via `kableExtra` (pour les sorties en pdf) & font l'export en .xls (optionnel).
